...
function buildMeetingInfo(url, htmlContents, sectionData = []) {
   // DEBUG
    ...
    return {
      ...
      venue2:       doc$(".nat_eve_infos p:contains('Impianto') .vle").length > 1 ? $(".nat_eve_infos p:contains('Impianto') .vle").last().text().trim() : '',
      address2:     doc$(".nat_eve_infos p:contains('Sede') .vle").length > 1 ? $(".nat_eve_infos p:contains('Sede') .vle").last().text().trim() : '',
